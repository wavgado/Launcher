...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/wavgado/launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `새로운 ${isPrerelease(data.version) ? '프리뷰버전' : '정식버전'} 사용가능`
        ...
        settingsUpdateButtonStatus('업데이트 확인', false, () => {
            if(!isDev){
                ...
                settingsUpdateButtonStatus('업데이트를 확인하는 중..', true)
            }
        })
    }
}
