    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    '호환되는<br>자바 설치를 찾을 수 없음',
                    '서버에 접속하려면 자바 8을 설치해야 합니다.',
                    '자동으로 설치',
                    '매뉴얼대로 설치'
                )
                setOverlayHandler(() => {
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            '오류<br>자바가 필요합니다',
                            '게임을 시작하려면 자바가 필요합니다. <br>https://java.com/ko/ 에서 설치를 진행해주세요.',
                            '이해했습니다',
                            '뒤로가기'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    '알 수 없는 오류:<br>자바 설치 실패',
                    '자동 자바 설치에 실패하였습니다. 하단 링크로 이동하여 자바를 설치해주세요. <br>https://java.com/ko/',
                    '이해했습니다'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    setLaunchDetails('버전 정보 가져오는 중..')
                    break
                case 'version':
                    setLaunchPercentage(40, 100)
                    loggerLaunchSuite.log('Version data loaded.')
                    setLaunchDetails('에셋 무결성 검증중..')
                    break
                case 'assets':
                ...
                case 'libraries':
                    ...
                    setLaunchDetails('기타 파일 무결성 검증중..')
                    break
                case 'files':
                    setLaunchPercentage(100, 100)
                    loggerLaunchSuite.log('File validation complete.')
                    setLaunchDetails('파일 다운로드중..')
                    break
            }
        } else if(m.context === 'progress'){
            switch(m.data){
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            '다운로드 오류',
                            '다운로드 서버에 연결할 수 없습니다. 인터넷 연결을 확인하시고 다시 시도해주시기 바랍니다.'
                        )
                    } else {
                        showLaunchFailure(
                            '다운로드 오류',
                            '알 수 없는 오류가 발생했습니다. (Ctrl + Shift + I 로 오류확인)'
                        )
                    }
